...
const debounce = <T extends (...args: any) => any>(func: T, wait: number) => {
	...
	return (args?: Parameters<T>) => {
		...
		d = setTimeout(() => {
			if (refCard.isValid() && refCard.getStackSize() === 1) func.apply(args);
		}, wait);
	};
};
...
refCard.onCreated.add(() => {
	refCard.setName("");
	debounceCardDetails();
});

refCard.onInserted.add(() => {
	refCard.setName("");
	refCard.removeUI(0);
});

refCard.onRemoved.add(() => {
	refCard.setName("");
	debounceCardDetails();
});

const createTokens: (button: Button, player: Player) => void = (button, player) => {
	let setting = button.getText() === "Tokens";
	const selfURL = refCard.getCardDetails().textureOverrideURL;
	...
	if (sf_id !== undefined) {
		fetch(`https://api.scryfall.com/cards/${sf_id}`)
			.then((r) => r.json())
			.then(async (v) => {
				const position = refCard.getPosition();
				const { all_parts } = v;
				...
				for (let index = 0; index < all_parts.length; index++) {
					...
					if (main_stack === undefined) {
						...
						// @ts-expect-error
						const q: Card = world.createObjectFromTemplate("31E5DB224CB620FF0B35E79BB7BB8D02", position.add([0, 8, 1]));
...
const transformCard: (button: ImageButton, player: Player) => void = () => {
	const selfURL = refCard.getCardDetails().textureOverrideURL;
	const [front_face] = selfURL.match(/(?<=&front_face=)(false|true)/g) ?? ["true"];
	const [sf_id] = selfURL.match(/[0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}/g) ?? [];
	if (sf_id !== undefined) {
...
function makeDetailedDescription(layout: CardLayout, easy_face: Partial<ScryfallCard>, [front, back]: [CardFace?, CardFace?] = [], front_face = true) {
	...
	refCard.addUI(StatCounterContainer);
	...
	if (layout === "normal") {
		...
		if ((easy_face["type_line"] ?? '').includes("Planeswalker")) {
			...
			refCard.addUI(LoyaltyContainer);
			refCard.addCustomAction("Add 1 Loyalty", undefined, "add_one_loyalty");
			refCard.addCustomAction("Lose 1 Loyalty", undefined, "lose_one_loyalty");
			refCard.addCustomAction("Add 5 Loyalty", undefined, "add_five_loyalty");
			refCard.addCustomAction("Lose 5 Loyalty", undefined, "lose_five_loyalty");
		}
	} else if (layout === "saga") {
		...
		RightBorder.setChild(new RichText().setText(Canvas_Text).setFontSize(24).setAutoWrap(true).setTextColor([0, 0, 0, 1])); //, 0, 0, 760, 1500);
		refCard.setTextureOverrideURL(front_face ? front.image_uris.normal.concat("&front_face=true") : back.image_uris.normal.concat("&front_face=false"));
		if (use_l_d) {
			refCard.addCustomAction("Add 1 " + (use_l_d === "pw" ? "Loyalty" : "Defense"), undefined, "add_one_loyalty");
			refCard.addCustomAction("Lose 1 " + (use_l_d === "pw" ? "Loyalty" : "Defense"), undefined, "lose_one_loyalty");
			refCard.addCustomAction("Add 5 " + (use_l_d === "pw" ? "Loyalty" : "Defense"), undefined, "add_five_loyalty");
			refCard.addCustomAction("Lose 5 " + (use_l_d === "pw" ? "Loyalty" : "Defense"), undefined, "lose_five_loyalty");
		} else {
			const loyaltyElement = refCard.getUIs().find((ui) => ui.position.equals(new Vector(-3.5, -2.13, -0.1), 1));
			const defenseElement = refCard.getUIs().find((ui) => ui.position.equals(new Vector(4.5, -2.4, -0.1), 1));
			if (!!loyaltyElement) refCard.removeUIElement(loyaltyElement);
			if (!!defenseElement) refCard.removeUIElement(defenseElement);
			const LoyaltyText = new Button();
			LoyaltyText.setFontSize(48);
			...
			globalState.loyaltyUI = LoyaltyText;
			refCard.addUI(LoyaltyContainer);
			refCard.removeCustomAction("add_one_loyalty");
			refCard.removeCustomAction("lose_one_loyalty");
			refCard.removeCustomAction("add_five_loyalty");
			refCard.removeCustomAction("lose_five_loyalty");
		}
	...
	refCard.addCustomAction("Add 1 Stat Counter", undefined, "add_one_stat");
	refCard.addCustomAction("Lose 1 Stat Counter", undefined, "lose_one_stat");
	refCard.addCustomAction("Add 5 Stat Counters", undefined, "add_five_stat");
	refCard.addCustomAction("Lose 5 Stat Counters", undefined, "lose_five_stat");

	RightSide.widget = RightBorder;
	LeftSide.widget = LeftBorder;
	refCard.addUI(RightSide);
	refCard.addUI(LeftSide);
}

function generateCardDetails() {
	process.nextTick(() => {
		if (refCard.getStackSize() > 1) return;
		...
		const [front_face] = refCard.getCardDetails()?.textureOverrideURL?.match(/(?<=&front_face=)(false|true)/g) ?? ["true"];
		if (sf_id !== undefined) {
			fetch(`https://api.scryfall.com/cards/${sf_id}`)
				.then((r) => r.json())
				.then((v: ScryfallCard) => {
					...
					refCard.onCustomAction.add(onCustomAction);
				});
		}
	});
}

const onCustomAction: (object: Card, player: Player, identifier: string) => void = (self, player, action_id) => {
	switch (action_id) {
	...
	if (globalState.stats !== 0) globalState.statsUI?.setVisible(true);
	else globalState.statsUI?.setVisible(false);
