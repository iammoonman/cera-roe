...
import ReactFlow, { Controls, Edge, Handle, MiniMap, Node, Position } from "react-flow-renderer";
...
const TourneyContainer = (props: Props) => {
	var edges: Edge[] = [];
	var nodes: Node[] = props.tournament.nodes.map((d, i, a) => {
		edges.push(
			...d.feeds.reduce((v: Edge[], q) => {
				// If the edge feeds from a winner into a loser, it shouldn't appear.
				var feedsLoser = props.tournament.nodes.filter((y) => {
					return y.loser && q == y.bnid;
				}).length;
				//console.log(isLoser)
	...
	const day = new Date(props.tournament.date);
	return (
		<>
			<div className="blockbg" style={{ height: "850px", display: "flex", flexDirection: "column" }}>
				<div className="d-flex" style={{ justifyContent: "space-between" }}>
					<h4>{props.tournament.title}</h4>
					{props.tournament.tag == "dps" ? (
						<Link href={`/dps`}>
							<a>
								<div className="tagsymbol"></div>
							</a>
						</Link>
					) : (
						<></>
							...
							{props.tournament.players
								...
								.map((pl: EventPlayer, index) => (
									<tr className={`playerRow`} key={index}>
										<td>{pl.rank}</td>
										<td>
											<PlayerLink player={{ name: PlayerNames[pl.playerID], id: pl.playerID }} />
										</td>
										<td>{pl.seed}</td>
										<td>{pl.wins}</td>
										<td>{pl.losses}</td>
